import Link from "next/link";
import { requireAdmin } from "@/lib/admin";
import { createClient } from "@/lib/supabase/server";
import type { Destination } from "@/lib/supabase/types";
import { getCategoryMeta } from "@/lib/destination-categories";
import { accentBgClass } from "@/lib/brand-accent";
import { Button } from "@/components/ui/Button";
import { AdminDeleteButton } from "@/components/admin/AdminDeleteButton";
import { deleteDestination } from "./actions";

export default async function AdminDestinationsPage(props: PageProps<"/admin/destinations">) {
  await requireAdmin();
  const { error } = await props.searchParams;
  const supabase = await createClient();
  const { data } = await supabase.from("destinations").select("*").order("name");
  const destinations = (data ?? []) as Destination[];

  return (
    <main className="mx-auto w-full max-w-3xl flex-1 px-6 py-16">
      <Link href="/admin" className="text-sm font-medium text-foreground/60 hover:text-foreground">
        ← Back to Admin Dashboard
      </Link>

      {error && (
        <p className="mt-4 rounded-lg bg-brand-red/10 px-3.5 py-2.5 text-sm text-brand-red">
          {error}
        </p>
      )}

      <div className="mt-4 flex items-center justify-between">
        <h1 className="font-serif text-2xl font-semibold tracking-tight">Explore Assam</h1>
        <Button href="/admin/destinations/new" size="sm">
          New destination
        </Button>
      </div>

      <div className="mt-8 flex flex-col gap-3">
        {destinations.map((destination) => {
          const accent = getCategoryMeta(destination.category).accent;
          return (
            <div
              key={destination.id}
              className="flex items-center justify-between gap-4 rounded-xl border border-foreground/10 p-4"
            >
              <div className="flex items-center gap-3">
                <span className={`h-2 w-2 shrink-0 rounded-full ${accentBgClass[accent]}`} />
                <div>
                  <p className="font-medium">{destination.name}</p>
                  <p className="text-xs text-foreground/50">
                    {getCategoryMeta(destination.category).label} · {destination.district}
                  </p>
                </div>
              </div>
              <div className="flex shrink-0 gap-2">
                <Button href={`/admin/destinations/${destination.id}/edit`} variant="outline" size="sm">
                  Edit
                </Button>
                <AdminDeleteButton
                  action={deleteDestination}
                  id={destination.id}
                  confirmMessage={`Delete "${destination.name}"? This can't be undone.`}
                />
              </div>
            </div>
          );
        })}
      </div>
    </main>
  );
}
