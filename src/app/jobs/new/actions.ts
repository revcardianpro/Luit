...
export async function createJob(formData: FormData) {
  ...
  if (!title || !company || !description || !jobType) {
    redirectWithError("/jobs/new", "Please fill in all required fields.");
  }

  if (!jobTypes.includes(jobType as JobType)) {
    redirectWithError("/jobs/new", "Invalid job type.");
  }

  if (!applyUrl && !applyEmail) {
    redirectWithError("/jobs/new", "Provide at least one way to apply (URL or email).");
  }
  ...
  if (error || !data) {
    redirectWithError("/jobs/new", error?.message ?? "Something went wrong.");
  }
